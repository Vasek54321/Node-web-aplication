...
řádku. */
let str = `"${req.body.nazev}","${req.body.typ}","${req.body.rok}"\n`;
/* Pomocí modulu fs a metody appendFile dojde k přidání připraveného řádku (proměnná str) 
do uvedeného souboru */
fs.appendFile(path.join(__dirname, 'data/filmy.csv'), str, function (err) {
...
app.get("/filmy", (req, res) => {
    ...
    csvtojson({headers:['nazev','typ','rok']}).fromFile(path.join(__dirname, 
    'data/filmy.csv'))
    .then(data => {
